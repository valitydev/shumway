...
public class AccountUtils {
    ...
    public static double emulateRealTransfer(
        ...
        for (int j = 0; j < numberOfRounds; j++) {
            ...
            final long vality = valityAccs.get(random.nextInt(valityAccs.size()));
            final long merchant = merchantAccs.get(random.nextInt(merchantAccs.size()));

            final int transferId = j;

            executorService.submit(() -> makeRealTransfer(client, provider, merchant, vality, transferId));
        }
    ...
    public static void makeRealTransfer(
        ...
        Posting p1 = new Posting(provider, vality, 100, "RUB", "Desc");
        Posting p2 = new Posting(vality, merchant, 95, "RUB", "Desc");
